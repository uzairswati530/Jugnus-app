...
import { Check, ChevronRight, MapPin, Clock, Star, Phone, User, MessageSquare, Gift, Award, Share2 } from 'lucide-react';
import {
  BRANCHES,
  FEMALE_SERVICES,
  MALE_SERVICES,
          ...
          <div key={label} className="flex items-center flex-1">
            <div className="flex flex-col items-center">
              ...
              <span className={`text-xs mt-1.5 text-center leading-tight hidden md:block
                ${active ? 'text-gray-900 font-medium' : done ? 'text-green-600' : 'text-gray-400'}`}>
                {label}
              </span>
            </div>
            {i < STEP_LABELS.length - 1 && (
              <div className={`flex-1 h-0.5 mx-1 rounded ${step > num ? 'bg-green-500' : step === num ? 'bg-[#C5A044]' : 'bg-gray-200'}`} />
            )}
          </div>
        );
      })}
...
function Step1Branch({ booking, onSelect }: { booking: BookingState; onSelect: (b: typeof BRANCHES[0]) => void }) {
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-6">Select the Jugnu&apos;s Salon location you&apos;d like to visit.</p>
        ...
        {BRANCHES.map(branch => (
          <button
            ...
            className={`w-full text-left card-2 p-5 rounded-xl flex items-center justify-between transition-all
              ${booking.branch?.id === branch.id ? 'border-[#C5A044] bg-[rgba(197,160,68,0.1)]' : 'hover:border-[rgba(197,160,68,0.5)]'}`}
          >
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-lg bg-[rgba(197,160,68,0.1)] border border-[rgba(197,160,68,0.3)] flex items-center justify-center shrink-0 mt-0.5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">{branch.todayBookings} bookings today · {branch.activeQueue} in queue</p>
              </div>
            </div>
            <ChevronRight size={18} className={booking.branch?.id === branch.id ? 'text-[#C5A044]' : 'text-gray-300'} />
          </button>
        ))}
      </div>
    </div>
...
function Step2Service({ booking, onSelect }: { booking: BookingState; onSelect: (s: Service) => void }) {
  const isMale = booking.branch?.id === 'e11';
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-6">
        Showing {isMale ? 'male' : 'female'} services for <span className="text-gray-900">{booking.branch?.name}</span>
      </p>
        ...
        {services.map(service => (
          <button
            ...
            className={`w-full text-left card-2 p-4 rounded-xl flex items-center justify-between transition-all
              ${booking.service?.name === service.name ? 'border-[#C5A044] bg-[rgba(197,160,68,0.1)]' : 'hover:border-[rgba(197,160,68,0.5)]'}`}
          >
            <div>
              <p className="font-semibold text-gray-900">{service.name}</p>
              <div className="flex items-center gap-3 mt-1">
                <span className="text-xs text-gray-400 flex items-center gap-1"><Clock size={11} />{service.duration} min</span>
                <span className="text-sm font-bold text-[#C5A044]">{formatPKR(service.price)}</span>
              </div>
            </div>
            {booking.service?.name === service.name && (
              <div className="w-6 h-6 rounded-full bg-[#C5A044] flex items-center justify-center shrink-0">
...
function Step3Stylist({ booking, onSelectStylist, onSelectSlot }:
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-900 mb-2">Choose Stylist & Time</h2>
        ...
        {branchStylists.map(stylist => (
          <div
            ...
            className={`card-2 p-4 rounded-xl transition-all cursor-pointer
              ${booking.stylist?.id === stylist.id ? 'border-[#C5A044]' : 'hover:border-[rgba(197,160,68,0.5)]'}`}
            onClick={() => onSelectStylist(stylist)}
          >
            <div className="flex items-center gap-3 mb-3">
              <div className="w-9 h-9 rounded-full bg-[rgba(197,160,68,0.1)] border border-[rgba(197,160,68,0.3)] flex items-center justify-center shrink-0">
                <User size={16} className="text-[#C5A044]" />
              </div>
              <div className="flex-1">
                <p className="font-semibold text-gray-900 text-sm">{stylist.name}</p>
                <p className="text-xs text-gray-400 flex items-center gap-1"><Star size={10} className="text-amber-500" />{stylist.specialty}</p>
              </div>
              {booking.stylist?.id === stylist.id && (
                <div className="w-5 h-5 rounded-full bg-[#C5A044] flex items-center justify-center">
  ...
  return (
    <div>
      ...
      <div className="card-2 p-4 rounded-xl mb-6 space-y-2.5">
        <p className="text-xs text-gray-400 uppercase tracking-wider font-medium mb-3">Booking Summary</p>
        {[
          ...
          ...(appliedDiscount > 0 ? [
            { label: 'Discount Applied', value: `- ${formatPKR(appliedDiscount)}`, highlight: true, color: 'text-green-600' },
            { label: 'Final Amount', value: formatPKR(finalPrice), highlight: true, color: 'text-[#C5A044]' }
          ] : [])
        ].map(item => (
          <div key={item.label} className="flex justify-between text-sm">
            <span className="text-gray-400">{item.label}</span>
            <span className={`font-medium text-right ${item.color || 'text-gray-900'}`}>{item.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Redemption Code (Optional)</label>
          <div className="flex gap-2">
            ...
            />
            {appliedDiscount > 0 && (
              <div className="flex items-center gap-2 px-3 py-3 bg-green-50 border border-green-300 rounded-lg">
                <Check size={18} className="text-green-600" />
                <span className="text-sm font-semibold text-green-600">{formatPKR(appliedDiscount)}</span>
              </div>
            )}
          </div>
          <p className="text-xs text-gray-400 mt-1">Have a redemption code? Enter it here to apply discount</p>
        </div>
      </div>
...
function BookingConfirmed({ booking, onReset, pointsEarned }: { booking: BookingState; onReset: () => void; pointsEarned: PointsEarned | null }) {
  ...
  return (
    <div className="text-center space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-2">Your appointment has been successfully scheduled.</p>
      </div>
      ...
      {pointsEarned && (
        <div className="card p-5 rounded-xl text-left">
          <div className="flex items-center gap-2 mb-4">
            <Award size={16} className="text-[#C5A044]" />
            <p className="font-semibold text-gray-900 text-sm">Loyalty Points Earned</p>
          </div>
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-3xl font-bold text-[#C5A044]">+{pointsEarned.amount.toLocaleString()}</p>
              <p className="text-xs text-gray-400 mt-0.5">Points added to your balance</p>
            </div>
          ...
          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-[#C5A044] rounded-full" style={{ width: `${((pointsEarned.totalTierRequirement - pointsEarned.pointsNeeded) / pointsEarned.totalTierRequirement) * 100}%` }} />
          </div>
          <p className="text-xs text-gray-400 mt-2">{pointsEarned.pointsNeeded.toLocaleString()} points to next tier</p>
        </div>
      )}
  ...
  return (
    <div className="space-y-5">
      <div className="card p-5 rounded-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-gray-900 flex items-center gap-2"><Gift size={16} className="text-[#C5A044]" />Your Loyalty Points</h3>
          <span className={`text-xs font-bold px-3 py-1 rounded-full border ${tc.bg} ${tc.text} ${tc.border}`}>{tier}</span>
        </div>
        <p className="text-4xl font-bold text-gray-900">{totalPoints.toLocaleString()}<span className="text-lg font-normal text-gray-400 ml-2">pts</span></p>
        <div className="mt-3 h-2 bg-gray-200 rounded-full overflow-hidden">
          <div className="h-full bg-[#C5A044] rounded-full" style={{ width: `${Math.min((totalPoints / 5000) * 100, 100)}%` }} />
        </div>
      ...
      {generatedCodes.length > 0 && (
        <RedemptionHistory
          redemptions={generatedCodes.map((c) => ({
            date: c.date,
            code: c.code,
      ...
      <div className="card p-5 rounded-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-gray-900 text-sm">Visit History</h3>
        ...
        <div className="space-y-3">
          {LOYALTY_HISTORY.map((h, i) => (
            <div key={i} className="flex items-center justify-between text-sm">
              <div>
                <p className="text-gray-900 font-medium">{h.service}</p>
                <p className="text-xs text-gray-400">{h.date}</p>
              </div>
              <div className="text-right">
                <p className="text-green-600 font-semibold">+{h.points.toLocaleString()} pts</p>
                <p className="text-xs text-gray-400">{formatPKR(h.amount)}</p>
              </div>
            </div>
          ))}
        </div>
...
export default function CustomerBookingView() {
  ...
  const handleWhatsApp = () => {
    ...
    window.open(`https://wa.me/923429309166?text=${encodeURIComponent(lines.join('\n'))}`, '_blank');
  };
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      <div className="flex gap-1 mb-6 p-1 rounded-lg bg-[#F5F5F5] border border-[rgba(197,160,68,0.3)] w-fit">
