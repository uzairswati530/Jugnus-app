...
import { Plus, Edit, Trash2, X, Star, Calendar } from 'lucide-react';
import { STAFF_DATABASE, BRANCHES, formatPKR, type StaffMember } from '@/lib/constants';
...
const RECENT_BOOKINGS = [
  { date: '02 Jun 2026', customer: 'Fatima Zaidi', service: 'Bridal Makeup (Signature)', time: '2:00 PM' },
  { date: '01 Jun 2026', customer: 'Sana Riaz', service: 'Party Makeup (Senior)', time: '5:30 PM' },
    ...
    <div className="p-6 space-y-6">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="card p-6 max-w-sm mx-4 space-y-4">
            <h3 className="text-lg font-bold text-gray-900">Delete Stylist?</h3>
      ...
      {selectedStaff && (
        <div className="fixed inset-0 z-40 flex items-center justify-end">
          <div onClick={() => setSelectedStaff(null)} className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="card w-full max-w-md h-screen overflow-y-auto relative">
            <div className="sticky top-0 flex items-center justify-between p-6 border-b border-[rgba(197,160,68,0.2)] bg-[#F5F5F5] z-10">
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-lg bg-[rgba(197,160,68,0.1)] border border-[rgba(197,160,68,0.3)] flex items-center justify-center text-2xl font-bold text-[#C5A044]">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{selectedStaff.branch}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-gray-900">{selectedStaff.phone}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">Status</span>
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${selectedStaff.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
              ...
              <div className="card-2 p-4 rounded-lg space-y-3">
                <h4 className="text-sm font-bold text-gray-900">This Month</h4>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-xs text-gray-500 mb-1">Bookings</p>
                    <p className="text-2xl font-bold text-[#C5A044]">{selectedStaff.bookingsThisMonth}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-1">Revenue</p>
                    <p className="text-xl font-bold text-green-600">{formatPKR(selectedStaff.revenueThisMonth)}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-xs text-gray-500 mb-1">Average Rating</p>
                    <div className="flex items-center gap-2">
                      <p className="text-lg font-bold text-yellow-500">{selectedStaff.avgRating}</p>
                      <div className="flex">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} size={12} className={i < Math.round(selectedStaff.avgRating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} />
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  <Calendar size={16} className="text-[#C5A044]" />
                  Recent Bookings
                </h4>
              ...
              <div className="space-y-2 pt-4 border-t border-[rgba(197,160,68,0.2)]">
                <button
                ...
                <button
                  ...
                  className="btn-ghost w-full py-2.5 text-sm flex items-center justify-center gap-2 text-red-600 hover:text-red-500"
                >
                  <Trash2 size={16} />
                  Delete
                </button>
          ...
          <tbody>
            {staff.map((member, i) => (
              <tr
                ...
                className={`border-b ${i < staff.length - 1 ? 'border-gray-100' : 'border-transparent'} hover:bg-[rgba(197,160,68,0.05)] transition-colors cursor-pointer ${member.status === 'Inactive' ? 'opacity-50' : ''}`}
              >
                <td className="py-4 px-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded bg-[rgba(197,160,68,0.1)] border border-[rgba(197,160,68,0.3)] flex items-center justify-center text-xs font-bold text-[#C5A044]">
                      {member.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <span className="font-medium text-gray-900">{member.name}</span>
                  </div>
                </td>
                <td className="py-4 px-3 text-gray-500">{member.branch}</td>
                <td className="py-4 px-3 text-gray-500 text-xs">{member.specialty}</td>
                <td className="py-4 px-3 text-center">
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${member.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                    {member.status}
                  </span>
                </td>
                <td className="py-4 px-3 text-center font-bold text-gray-900">{member.bookingsThisMonth}</td>
                <td className="py-4 px-3 text-right font-bold text-green-600">{formatPKR(member.revenueThisMonth)}</td>
                <td className="py-4 px-3 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <span className="text-sm font-bold text-yellow-500">{member.avgRating}</span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm overflow-y-auto py-4">
      <div className="card w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-gray-900">{staff ? 'Edit Stylist' : 'Add Stylist'}</h3>
