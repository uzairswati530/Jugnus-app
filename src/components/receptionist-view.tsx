...
import LoyaltyLookupModal from './loyalty-lookup-modal';
import PointsAwardPopup from './points-award-popup';
...
function TierBadge({ tier }: { tier: 'Bronze' | 'Silver' | 'Gold' }) {
  ...
  return (
    <span className={`text-xs font-bold px-2 py-0.5 rounded-full border border-current/20 ${colors.bg} ${colors.text}`}>
      {tier}
    </span>
  );
}
...
function WalkInModal({ onClose, onAdd }: WalkInModalProps) {
  ...
  const services = [
    ...
    'Wash & Blow Dry', 'Party Makeup (Senior)', 'Acrylic Nails', 'Facial (Basic)',
    'Facial (Whitening)', 'Hair Color', 'Hair Color (Global)', 'Head Massage',
  ];
  const stylists = ['Ayesha Khan', 'Sara Ahmed', 'Nadia Malik', 'Zara Hussain', 'Maria Qureshi', 'Usman Ali'];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="card w-full max-w-md mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-gray-900">Add Walk-in</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onChange={e => setForm(f => ({ ...f, loyaltyMember: e.target.checked, loyaltyPhone: '' }))}
          ...
          {form.loyaltyMember && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Enter Phone to Load Profile</label>
          ...
          {loyaltyFound && (
            <div className="card-2 p-3 rounded-lg space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-gray-900">{loyaltyFound.name}</p>
                <TierBadge tier={loyaltyFound.tier} />
              </div>
              <p className="text-xs text-gray-400">{loyaltyFound.points.toLocaleString()} points available</p>
              {loyaltyFound.points >= 500 && (
                <label className="flex items-center gap-2 cursor-pointer pt-1">
                  <input
                    ...
                    className="w-3 h-3 rounded accent-[#C5A044] cursor-pointer"
                  />
                  <span className="text-xs font-medium text-green-600">Redeem Points (Rs 500 off)</span>
                </label>
              )}
            </div>
          )}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Stylist</label>
            <select
              className="input-dark w-full px-4 py-2.5"
              value={form.stylist}
...
export default function ReceptionistView() {
    ...
    setAppointments(prev => prev.map(a => {
      ...
      if (next === 'Done') {
        ...
        if (appointment?.tierBadge) {
          const servicePrice = appointment.service === 'Bridal Makeup (Signature)' ? 40000 :
                               appointment.service === 'Party Makeup (Senior)' ? 10000 : 2000;
  ...
  const cycleQueueStatus = (id: string) => {
    ...
    setQueue(prev => prev.map(w => {
      ...
      if (next === 'Done' && w.tierBadge) {
        const servicePrice = w.service === 'Haircut' ? 1500 : w.service === 'Haircut + Beard' ? 2000 : 800;
  ...
  const markDone = (id: string) => {
    setQueue(prev => prev.map(w => w.id === id ? { ...w, status: 'Done' as WalkInStatus } : w));
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <h2 className="text-xl md:text-2xl font-bold text-gray-900">Receptionist Dashboard</h2>
        <button
          onClick={() => setShowLoyaltyLookup(true)}
          className="btn-ghost flex items-center gap-2 px-4 py-2.5 text-sm self-start sm:self-auto"
        >
          <Award size={16} />
          Loyalty Lookup
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-5">
            <Calendar size={18} className="text-[#C5A044]" />
            <h2 className="text-lg font-bold text-gray-900">Today&apos;s Appointments</h2>
            <span className="ml-auto text-xs font-medium bg-[rgba(197,160,68,0.1)] text-[#C5A044] border border-[rgba(197,160,68,0.3)] px-2 py-0.5 rounded-full">
              {appointments.length}
            </span>
          </div>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-5">
            <Users size={18} className="text-[#C5A044]" />
            <h2 className="text-lg font-bold text-gray-900">Walk-in Queue</h2>
            <span className="ml-auto text-xs font-medium bg-[rgba(197,160,68,0.1)] text-[#C5A044] border border-[rgba(197,160,68,0.3)] px-2 py-0.5 rounded-full">
              ...
              <div key={w.id} className={`card-2 p-4 flex flex-col gap-2 ${w.status === 'Done' ? 'opacity-50' : ''}`}>
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <span className="mt-0.5 w-6 h-6 rounded-full bg-[rgba(197,160,68,0.1)] border border-[rgba(197,160,68,0.3)] text-[#C5A044] text-xs font-bold flex items-center justify-center shrink-0">
                      {i + 1}
                    </span>
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm text-gray-500">{w.service}</p>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {[
          { label: 'Total Bookings', value: totalBookings, icon: <Calendar size={18} />, color: 'text-blue-600' },
          { label: 'Total Walk-ins', value: totalWalkIns, icon: <Users size={18} />, color: 'text-amber-600' },
          { label: 'In Chair', value: inChair, icon: <Scissors size={18} />, color: 'text-[#C5A044]' },
          { label: 'Completed', value: completed, icon: <CheckCircle size={18} />, color: 'text-green-600' },
        ].map(stat => (
          <div key={stat.label} className="card p-3 md:p-4 flex items-center gap-3 md:gap-4">
