    ...
    return (
      <div className="card p-6 rounded-xl">
        <div className="text-center space-y-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-900">Redemption Code Generated!</h3>
            <p className="text-gray-500 text-sm mt-1">{generatedCode.pointsUsed.toLocaleString()} points redeemed</p>
          </div>

          <div className="bg-[#F5F5F5] border border-[rgba(197,160,68,0.3)] rounded-lg p-6">
            <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Your Redemption Code</p>
            <div className="flex items-center justify-between gap-3">
              <p className="text-3xl font-bold text-[#C5A044] font-mono">{generatedCode.code}</p>
              <button
                onClick={copyCode}
          ...
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                <p className="text-amber-700 font-semibold">Show this code to the receptionist</p>
                <p className="text-gray-500 text-xs mt-1">before payment to apply your Rs {generatedCode.discountAmount.toLocaleString()} discount.</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="card-2 p-3 rounded-lg text-center">
              ...
              <p className="text-lg font-bold text-[#C5A044] mt-1">{generatedCode.pointsUsed.toLocaleString()}</p>
            </div>
            <div className="card-2 p-3 rounded-lg text-center">
              <p className="text-xs text-gray-500">Discount</p>
              <p className="text-lg font-bold text-green-600 mt-1">Rs {generatedCode.discountAmount.toLocaleString()}</p>
            </div>
          </div>
      ...
      <div className="card p-6 rounded-xl space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
            <Gift size={18} className="text-[#C5A044]" />
            Redeem Points
          </h3>
          <button
        ...
        {!canRedeem && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
            <p className="text-sm text-red-600">Minimum 500 points required to redeem. You have {pointsBalance.toLocaleString()} points.</p>
          </div>
        )}
        ...
        {canRedeem && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-3">Points to Redeem</label>
              <input
                type="range"
                min={500}
                max={maxRedeemable}
                ...
                className="w-full accent-[#C5A044]"
              />
              <div className="flex items-center justify-between mt-2 text-sm">
                <span className="text-gray-500">500 points min</span>
                <span className="text-[#C5A044] font-bold">{pointsToRedeem.toLocaleString()} points</span>
                <span className="text-gray-500">{maxRedeemable.toLocaleString()} max</span>
              </div>
            </div>

            <div className="bg-[rgba(197,160,68,0.1)] border border-[rgba(197,160,68,0.3)] rounded-lg p-4">
              <p className="text-xs text-gray-500 mb-1">Discount Amount</p>
              <p className="text-3xl font-bold text-[#C5A044]">Rs {discountAmount.toLocaleString()}</p>
              <p className="text-xs text-gray-400 mt-2">Conversion: 100 points = Rs 100</p>
            </div>
    ...
    <div className="card p-5 rounded-xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-3xl font-bold text-[#C5A044] mt-1">{Math.floor(pointsBalance / 100) * 100} points</p>
