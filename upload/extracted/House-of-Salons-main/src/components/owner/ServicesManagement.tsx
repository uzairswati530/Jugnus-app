...
import { SERVICES_DATABASE, BRANCHES, formatPKR, type ServiceItem } from '../../data/constants';
...
export default function ServicesManagement({ onServiceUpdate }: ServicesManagementProps) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 bg-surface rounded-lg border border-primary-border w-fit">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-primary-border">
              ...
              <th className="text-center py-3 px-3 text-gray-400 font-medium">Duration</th>
              <th className="text-right py-3 px-3 text-gray-400 font-medium">Price</th>
          ...
          <tbody>
            {tabServices.map((service, i) => (
              <tr key={service.id} className={`border-b ${i < tabServices.length - 1 ? 'border-white/5' : 'border-transparent'} hover:bg-primary/5 transition-colors ${service.status === 'Inactive' ? 'opacity-50' : ''}`}>
                <td className="py-4 px-3 font-medium text-white">{service.name}</td>
                <td className="py-4 px-3">
                  <span className={`text-xs font-semibold ${categoryColors[service.category]}`}>{service.category}</span>
                </td>
                <td className="py-4 px-3 text-center text-gray-400">{service.duration} min</td>
                  ...
                  {editingPriceId === service.id ? (
                    <div className="flex gap-1">
                      <input
                        ...
                        onKeyPress={e => e.key === 'Enter' && handlePriceSave(service.id)}
                        autoFocus
                      />
                      <button
                    ...
                    <span
                      ...
                      className="text-primary font-bold cursor-pointer hover:text-orange-400 transition-colors"
                    >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm overflow-y-auto py-4">
      <div className="card w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-white">{service ? 'Edit Service' : 'Add New Service'}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Available at Branches</label>
